...
TEST(Obstacle, TestCantMove) {
  ...
  // /test/samples
  path += "/test/bag/";
  std::string file = path + "obstacle.bag";
  ...
  //! Create an object of LaserProcessing class as we will use the public
  //! function of that object to run tests against
  // LaserProcessing laserProcessing(*laserScan);
  // geometry_msgs::Point point = laserProcessing.detectPositionClosest();
  bool res = Processing::checkHeadingIsClear(*odom, *laserScan, 8);
  ASSERT_EQ(res, false);

  // We check the closest point knowing that this is indeed the actual value,
  // refer to quiz 5 for more examples EXPECT_NEAR(point.x,0.0,0.01);
  // EXPECT_NEAR(point.y,4.12,0.01);
}

TEST(Obstacle, TestCanMove) {
  ...
  // LaserProcessing laserProcessing(*laserScan);
  // geometry_msgs::Point point = laserProcessing.detectPositionClosest();
  bool res = Processing::checkHeadingIsClear(*odom, *laserScan, 6);
  ASSERT_EQ(res, true);
